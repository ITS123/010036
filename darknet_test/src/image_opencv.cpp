    ...
    void socket_detector(char *datacfg, char *cfgfile, char *weightfile, float thresh, float hier_thresh, char *outfile, int fullscreen, int port_num)
    {
        ...
        while (1)
        {
            ...
            while (1)
            {
                ...
                {
                    if (recv(client_socket, sockData, imgSize, 0) == -1)
                        return;
                ...
                Point pts1[4] = {Point(COL * 0.3 / 2, ROW * 0.65),
                                Point(COL * 0.7 / 2, ROW * 0.65),
                                Point(COL / 2, ROW),
                                Point(0, ROW)};

                const Point *polygons[1] = {pts1};
                int npts[1] = {4};
                polylines(img, polygons, npts, 2, true, Scalar(0, 0, 255));
